...
const registerUser = asyncHandler( async (req,res) =>{ // we usee async handler because inside the function we have so much asynchronous operations , if anyone threw an error asynchandler cathes automatically 
    ...
    if(
        ...
        .some((field )=>field?.trim()==="" // does it have atleast one element that satisfy the conditon
        // withjout this ?. it cannot read the property of trim
        )
    ) {
    ...
    const avatarLocalPath = req.files?.avatar?.[0]?.path; // this is a string "./public/temp/avatar-172534534.png"
